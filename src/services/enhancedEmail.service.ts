...
class EnhancedEmailService {
    ...
    try {
      ...
      if (includeThreads) {
        ...
        if (useEnhancedUnification) {
          threads = EnhancedEmailIntegrationService.unifyEnhancedThread(threadsData, "gmail");
        ...
        if (useEnhancedUnification) {
          emails = EnhancedEmailUnificationServiceExtended.unifyEmailArray(
            messagesData,
            "gmail",
  ...
  async fetchOutlookEmailsByCategory(account: any, category: string, options: any) {
    ...
    try {
      ...
      // Unify emails with enhanced features if requested
      if (useEnhancedUnification) {
        emails = EnhancedEmailUnificationServiceExtended.unifyEmailArray(messagesData, "outlook", category);
      } else {
      ...
      if (includeThreads) {
        ...
        threads = Array.from(conversationMap.entries()).map(([convId, threadEmails]: [string, any[]]) => {
          ...
          if (useEnhancedUnification) {
            return EnhancedEmailUnificationServiceExtended.unifyEnhancedThread(baseThread, "outlook", sortedEmails);
          }
  ...
  async fetchThreadById(accountId: string, threadId: string, provider: string, useEnhancedUnification = true) {
    try {
      ...
      if (this.threadCache.has(cacheKey)) {
        ...
        if (useEnhancedUnification) {
          return EnhancedEmailUnificationServiceExtended.unifyEnhancedThread(cachedThread, provider);
        }
      ...
      if (useEnhancedUnification) {
        return EnhancedEmailUnificationServiceExtended.unifyEnhancedThread(threadData, provider);
      }
  ...
  async sendGmailEmail(account: any, emailData: any, useEnhancedUnification = true) {
    ...
    if (useEnhancedUnification) {
      draftData = EnhancedEmailUnificationServiceExtended.createEnhancedDraftEmail(emailData, "gmail");
    } else {
  ...
  async sendOutlookEmail(account: any, emailData: any, useEnhancedUnification = true) {
    ...
    if (useEnhancedUnification) {
      draftData = EnhancedEmailUnificationServiceExtended.createEnhancedDraftEmail(emailData, "outlook");
    } else {
  ...
  async replyToEmail(accountId: string, originalEmailId: string, replyData: any, useEnhancedUnification = true) {
    try {
      ...
      if (useEnhancedUnification) {
        replyEmailData = EnhancedEmailUnificationServiceExtended.createReplyEmail(
          originalEmail,
          replyData,
          account.provider
        );
  ...
  resolveEmailConflicts(unifiedEmail: any, accountId: string) {
    ...
    // Check for conflicts
    const conflicts = EnhancedEmailUnificationServiceExtended.detectConflict(
      unifiedEmail,
      existingEmail,
  ...
  async performOutlookEnhancedIncrementalSync(account: any, category: string, lastSyncTime: string) {
    ...
    try {
      ...
      const changedEmails = changes.map((email: any) => ({
        ...
        email: email["@removed"]
          ? null
          : EnhancedEmailUnificationServiceExtended.unifyEmail(email, "outlook", category),
      }));
