// Enhanced Email API Controller
// Handles all email operations with threading, categorization, and sync

const EnhancedEmailService = require("../services/enhancedEmail.service");
const emailService = new EnhancedEmailService();

class EnhancedEmailController {
  ...
  static async markEmailRead(req, res) {
    try {
  ...
  static async getEmailAnalytics(req, res) {
    try {
      const { accountId } = req.params;
      const { useEnhancedUnification = "true" } = req.query;
  ...
  static async bulkEmailActions(req, res) {
    try {
      const { accountId } = req.params;
  ...
  static calculateSyncQuality(syncStatus) {
  ...
  static estimateNextSync(syncStatus) {
...
module.exports = EnhancedEmailController;
