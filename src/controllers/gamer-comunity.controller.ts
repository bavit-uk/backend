...
export const gamercomunityController = {
  addgamercomunity: async (req: Request, res: Response) => {
    try {
      const { title, description, image, privacy, isBlocked } = req.body;
      console.log(title, description, image, privacy, isBlocked);
      const newgamercomunity = await GamerComunityService.creategamercomunity(title, description, image, privacy, isBlocked);
      res
  ...
  editgamercomunity: async (req: Request, res: Response) => {
    try {
      const { id } = req.params;
      const { title, description, image, privacy, isBlocked } = req.body;
      const gamercomunity = await GamerComunityService.editgamercomunity(id, { title, description, image, privacy, isBlocked });
